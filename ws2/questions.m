...
function e = errEst( A ) 
%Not working
%have to redo this need to best guess
%at End time with stepsize 1/8 and use that as the
%basis for calculating error- need to completely redo this function
global pop
S=0; %sum of dif squared
Assumed_solution = A(end,1);
for i=1:size(A,1)
    S = S + ( A(i,1) - Assumed_solution )^2;
end
e = sqrt( 1/size(A,1) * S );
